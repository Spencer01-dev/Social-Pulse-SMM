...
logger = logging.getLogger("socialpulse.providers.secsers")


class SecsersProvider(ProviderInterface):
    """
    Dedicated Secsers Reseller API v2 Provider Client.
    Connects to https://secsers.com/api/v2 for catalog synchronization,
    order placement, balance querying, and order status tracking.
    """
    ...
    def __init__(
        ...
        self.api_url = (api_url or settings.SECSERS_API_URL).rstrip("/")
        self.api_key = api_key or settings.SECSERS_API_KEY
        ...
        async with httpx.AsyncClient(timeout=self.timeout, follow_redirects=True, verify=False) as client:
            try:
                ...
                if isinstance(result, dict) and "error" in result:
                    ...
                    logger.warning(f"Secsers Provider API error: {err_msg}")
                    raise HTTPException(
                        status_code=status.HTTP_502_BAD_GATEWAY,
                        detail=f"Secsers Provider Error: {err_msg}",
                    )
                return result
            except HTTPException:
                raise
            except httpx.HTTPStatusError as exc:
                logger.error(f"Secsers HTTP status error {exc.response.status_code}")
                raise HTTPException(
                    status_code=status.HTTP_502_BAD_GATEWAY,
                    detail=f"Secsers HTTP Error {exc.response.status_code}",
                )
            except httpx.RequestError as exc:
                logger.error(f"Secsers connection error: {str(exc)}")
                raise HTTPException(
                    status_code=status.HTTP_504_GATEWAY_TIMEOUT,
                    detail=f"Failed to connect to Secsers: {str(exc)}",
                )

    async def get_services(self) -> List[ProviderServiceItem]:
        """Fetch full service catalog from Secsers."""
        ...
        for item in data:
            try:
                ...
                logger.warning(f"Skipping malformed Secsers service item {item}: {e}")
                continue

        return services

    async def get_balance(self) -> ProviderBalance:
        """Query live account balance from Secsers."""
        try:
            ...
            logger.error(f"Failed to query Secsers balance: {exc}")
    ...
    async def create_order(
        ...
        **kwargs: Any
        ...
        """Dispatch a single order to Secsers."""
        ...
        if not isinstance(data, dict) or "order" not in data:
            raise HTTPException(
                status_code=status.HTTP_502_BAD_GATEWAY,
                detail=f"Secsers returned unexpected order response: {data}",
            )
    ...
    async def get_order_status(self, provider_order_id: str) -> ProviderOrderStatus:
        """Poll order status from Secsers."""
    ...
    async def refill_order(self, provider_order_id: str) -> ProviderRefillResponse:
        """Trigger refill on an eligible order on Secsers."""
        data = await self._post_request({
            "action": "refill",
            "order": provider_order_id,
        })
    ...
    async def cancel_order(self, provider_order_id: str) -> Dict[str, Any]:
        """Request cancellation on Secsers."""
